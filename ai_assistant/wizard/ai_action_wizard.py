...
class AiActionWizard(models.TransientModel):
    ...
    def _get_catalog_context(self):
        ...
        # Productos (venta y compra)
        products = self.env['product.product'].search(
            [('active', '=', True)], limit=120)
        product_lines = []
        for p in products:
            ...
            product_lines.append(
                f'  - "{p.name}" | UOM: {uom} | precio venta: {price} | costo: {cost}')
    ...
    def _find_partner(self, name, supplier=False):
        ...
        # 1. Exacto
        p = Partner.search(domain + [('name', '=ilike', name)], limit=1)
        if p:
            return p
        # 2. Nombre comercial / display_name / ref
        p = Partner.search(domain + ['|', '|',
        ...
        for word in words:
            ...
            if p:
                _logger.info('Partner encontrado por palabra "%s": %s', word, p.name)
                return p
        ...
        raise UserError(
            _('No se encontró el contacto "%s".\n'
              'Verifique que exista en Odoo o use el nombre exacto.') % name
        )
    ...
    def _find_product(self, name):
        ...
        if words:
            ...
            if p:
                return p
        # 4. Mejor candidato por palabras (OR con scoring)
        if words:
