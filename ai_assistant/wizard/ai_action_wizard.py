...
class AiActionWizard(models.TransientModel):
    ...
    show_ref     = fields.Boolean(default=False)

    @api.onchange('action_type')
    def _onchange_action_type_flags(self):
        doc_types = {'create_out_invoice','create_in_invoice',
                     'create_out_refund','create_in_refund','create_sale_order'}
        self.show_lines   = self.action_type in doc_types
        self.show_partner = self.action_type in doc_types | {'send_email'}
        self.show_ref     = self.action_type in {'create_in_invoice','create_in_refund'}
    ...
    def default_get(self, fields_list):
        ...
        # Inicializar flags según action_type por defecto (create_out_invoice)
        res['show_lines']   = True
        res['show_partner'] = True
        res['show_ref']     = False
        return res
