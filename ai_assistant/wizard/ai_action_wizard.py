...
class AiActionWizard(models.TransientModel):
    ...
    def _find_partner(self, name, supplier=False):
        """
        Búsqueda SQL directa — 5 estrategias en cascada.
        Registra en log qué estrategia encontró el partner.
        """
        if not name:
        ...
        def fetch_partner(pid):
            return self.env['res.partner'].browse(pid)

        def log_found(label, pid, pname):
            _logger.info('[AI Partner] "%s" encontrado por %s -> id=%s "%s"', name, label, pid, pname)

        # ── 1. Exacto (case-insensitive) en name o commercial_company_name ─
        cr.execute("""
            SELECT id, name FROM res_partner
            WHERE active = true
              AND (
                lower(name) = lower(%s)
                OR lower(COALESCE(commercial_company_name, '')) = lower(%s)
                OR lower(COALESCE(ref, '')) = lower(%s)
              )
        ...
        row = cr.fetchone()
        if row:
            log_found('exacto', row[0], row[1])
            return fetch_partner(row[0])

        # ── 2. ILIKE parcial en todos los campos de texto clave ─────────────
        pct = '%' + name + '%'
        cr.execute("""
            SELECT id, name FROM res_partner
            WHERE active = true
              AND (
                name                                    ILIKE %s
                OR COALESCE(commercial_company_name,'') ILIKE %s
                OR COALESCE(display_name,'')            ILIKE %s
                OR COALESCE(ref,'')                     ILIKE %s
                OR COALESCE(email,'')                   ILIKE %s
                OR COALESCE(vat,'')                     ILIKE %s
              )
            ORDER BY
            ...
            LIMIT 1
        """, [pct, pct, pct, pct, pct, pct, name, pct])
        row = cr.fetchone()
        if row:
            log_found('ILIKE parcial', row[0], row[1])
            return fetch_partner(row[0])

        # ── 3. Cada token por separado — OR entre tokens, puntuar candidatos ─
        tokens = [t for t in name.split() if len(t) >= 2]
        if tokens:
            # Construir WHERE con OR para cada token en name Y commercial_company_name
            where_parts = []
            params = []
            for t in tokens:
                pt = '%' + t + '%'
            ...
            sql = """
                ...
                ORDER BY (customer_rank + supplier_rank) DESC
                LIMIT 40
            """.format(where=' OR '.join(where_parts))
            cr.execute(sql, params)
            candidates = cr.fetchall()

            if candidates:
                # Puntuar: cuántos tokens del input aparecen en el nombre candidato
                def score(row):
                    haystack = self._normalize(row[1] + ' ' + row[2])
                    return sum(1 for t in tokens if self._normalize(t) in haystack)

                best = max(candidates, key=score)
                if score(best) > 0:
                    log_found(f'tokens ({score(best)}/{len(tokens)})', best[0], best[1])
                    return fetch_partner(best[0])

        # ── 4. Comparación normalizada (sin tildes) sobre todos los partners ─
        ...
        """)
        candidates = cr.fetchall()
        best_id, best_name, best_score = None, '', 0
        for pid, pname, cname in candidates:
            pnorm = self._normalize(pname or '')
            cnorm = self._normalize(cname or '')
            s = 0
            if norm in pnorm or norm in cnorm:
                s = 10
            elif pnorm in norm or cnorm in norm:
                s = 8
            else:
                s = sum(1 for t in (tokens or [norm])
                        if self._normalize(t) in pnorm or self._normalize(t) in cnorm)
            if s > best_score:
                best_score = s
                best_id    = pid
                best_name  = pname
        if best_score > 0 and best_id:
            log_found(f'normalizado (score={best_score})', best_id, best_name)
            return fetch_partner(best_id)

        # ── 5. name_search nativo de Odoo ───────────────────────────────────
        results = self.env['res.partner'].name_search(name, limit=1)
        if results:
            p = self.env['res.partner'].browse(results[0][0])
            log_found('name_search', p.id, p.name)
            return p

        # ── Sin resultado — sugerir similares ───────────────────────────────
        cr.execute("""
            ...
            LIMIT 500
        """)
        all_names = [r[0] for r in cr.fetchall()]
        suggestions = sorted(
            all_names,
            key=lambda n: sum(1 for t in tokens if self._normalize(t) in self._normalize(n or '')),
            reverse=True
        )[:3]
        sug_text = (', '.join(suggestions)) if suggestions else ''

        raise UserError(
            _('No se encontró el contacto "%s" en Odoo.\n'
              'Contactos similares: %s') % (name, sug_text)
        )
